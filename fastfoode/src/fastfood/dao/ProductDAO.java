...
public class ProductDAO extends FastFoodDAO<Product, Integer> {
    ...
    public List<Product> getReStoreProduct(String user){
        String sql = "{CALL proc_showProductDelete(?)}";
        return this.getByIdStore(sql, user);
    }
    
    public int reStore(int idProduct){
        String sql = "update PRODUCT set EXISTSS = 1 where IDPRODUCT = ?";
        return JDBCHelper.update(sql, idProduct);
    }
